          ...
          <template #default="scope">
            <el-space>
              ...
              <el-button
                ...
                icon="Connection"
                circle
                @click="showRolePermissionDialog(scope.row)"
                title="菜单授权"
              ></el-button>
              <el-button
                size="small"
...
const loadRoles = async () => {
  const resp = await http.post(
    `/api/role/list?pageNum=${queryParams.pageNum}`,
    {},
    { roleCode: queryParams.roleCode, roleName: queryParams.roleName }
  );
  console.log("resp:", resp);
  roleList.value = resp.data.list;
...
const saveRole = () => {
  roleFormRef.value.validate(valid => {
    if (valid) {
      ...
      http.post("/api/role/save", payload).then(res => {
        if (res.code === 200) {
          search();
...
const deleteRole = item => {
  ElMessageBox.confirm(`确认删除角色【${item.roleName}】？`, "删除确认", {
    ...
    http.post(`/api/role/delete/${item.id}`).then(res => {
      if (res.code === 200) {
        search();
...
const showRolePermissionDialog = item => {
  ...
  http
    .post(`/api/role_permission/${item.id}`)
    .then(res => {
      if (res.code === 200) {
