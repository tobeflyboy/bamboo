package com.nutcracker.bamboo.interfaces.security.extension.wx;
...
public class WxMiniAppPhoneAuthenticationProvider implements AuthenticationProvider {
    ...
    public WxMiniAppPhoneAuthenticationProvider(SysUserService sysUserService, WxMaService wxMaService) {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        SysUser user = sysUserService.findByMobile(phoneNumber);

        if (user == null) {
            // 用户不存在，注册新用户
            boolean registered = sysUserService.registerUserByMobileAndOpenId(phoneNumber, openId);
            if (!registered) {
                throw new UsernameNotFoundException("用户注册失败");
            }
            // 重新获取用户信息
            user = sysUserService.findByMobile(phoneNumber);
        } else {
            // 用户存在，绑定openId（如果未绑定）
            sysUserService.bindUserOpenId(user.getUserId(), openId);
        }
