<template>
  <div>
    <!-- shadow 属性设置卡片阴影出现的时机。 该属性的值可以是：always、hover、never。默认值：always-->
    <el-card>
      <!-- 搜索区域 -->
      <el-row :gutter="15" style="margin-bottom: 15px">
        ...
        <el-col :span="4">
          <el-select v-model="data.status" placeholder="用户状态" clearable style="width: 100%">
            <el-option v-for="(value, key) in data.userStatus" :key="key" :label="value" :value="key" />
          </el-select>
        </el-col>
        <el-col :span="4">
          ...
          <template #default="scope">
            <el-space>
              ...
              <el-tooltip content="重置密码" placement="top">
                <el-button size="small" type="warning" icon="Key" circle @click="showResetPwdDialog(scope.row)" />
              </el-tooltip>

              <el-tooltip content="删除用户" placement="top">
                <el-button size="small" type="danger" icon="Delete" circle @click="deleteUser(scope.row)" />
              </el-tooltip>
            </el-space>
          </template>
        </el-table-column>
    ...
    >
      <el-form ref="currentFormRef" :model="data.currentForm" :rules="data.currentFormRules" label-width="100px">
        <el-form-item label="账号名称：" prop="username" v-if="data.addUserFormVisible">
          <el-input v-model.trim="data.currentForm.username" placeholder="请输入6-12位的英文字母或字符" />
        </el-form-item>

        <el-form-item label="真实姓名：" prop="realName" v-if="data.addUserFormVisible">
          <el-input v-model.trim="data.currentForm.realName" placeholder="请输入姓名" />
        </el-form-item>

        <template v-if="data.addUserFormVisible">
          <el-form-item label="密码：" prop="password">
            <el-input
              v-model.trim="data.currentForm.password"
          ...
          <el-form-item label="确认密码：" prop="confirmPassword">
            <el-input
              v-model.trim="data.currentForm.confirmPassword"
              placeholder="请再次输入密码"
              type="password"
              show-password
            />
        ...
        <el-form-item label="邮箱：" prop="email">
          <el-input v-model.trim="data.currentForm.email" placeholder="请输入邮箱地址" />
        </el-form-item>
        ...
        <el-form-item label="所属角色：" prop="roleId">
          <el-select v-model="data.currentForm.roleId" placeholder="请选择角色">
            <el-option v-for="item in data.roles" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
      </el-form>
    ...
    <!-- 重置密码对话框 -->
    <el-dialog title="重置密码" v-model="data.resetPwdFormVisible" width="500px" :close-on-click-modal="false">
      <el-form ref="resetPwdFormRef" :model="data.resetPwdForm" :rules="data.resetPwdFormRules" label-width="100px">
        ...
        <el-form-item label="密码：" prop="password">
          <el-input
            v-model.trim="data.resetPwdForm.password"
            placeholder="请输入6-12位的英文字母或字符"
            type="password"
            show-password
          />
        ...
        <el-form-item label="确认密码：" prop="confirmPassword">
          <el-input v-model.trim="data.resetPwdForm.confirmPassword" placeholder="请再次输入密码" type="password" show-password />
        </el-form-item>
      </el-form>
...
const initData = async () => {
  try {
    ...
    // 并行请求
    const [statusRes, rolesRes] = await Promise.all([http.post("/api/user/status-enum"), http.post("/api/role/all-list")]);
...
const submitForm = async () => {
  try {
    ...
    const params = isAdd
      ? {
          username: data.currentForm.username,
          realName: data.currentForm.realName,
          password: data.currentForm.password,
          email: data.currentForm.email,
          roleId: data.currentForm.roleId
        }
      : {
          userId: data.currentForm.userId,
          status: data.currentForm.status,
          email: data.currentForm.email,
          roleId: data.currentForm.roleId
        };
...
const resetPwd = async () => {
  try {
    ...
    const res = await http.post("/api/user/reset-pwd", {
      userId: data.resetPwdForm.userId,
      newPassword: data.resetPwdForm.password
    });
...
const deleteUser = user => {
  ElMessageBox.confirm(`确定要删除用户【${user.realName}】吗？`, "删除确认", {
    ...
    .then(async () => {
      const res = await http.post(`/api/user/delete/${user.userId}`);
      if (res.code === 200) {
...
const showExportUser = async () => {
  ...
  try {
    ...
    // 调用 API
    const result = await http.get("/api/user/export", {
      params,
      responseType: "blob"
    });
