<template>
  <el-card shadow="hover" class="main-container">
    ...
    <!-- 权限树 -->
    <el-tree :data="data.permissionTree" node-key="id" default-expand-all :props="data.defaultProps" class="custom-tree">
    ...
    >
      <el-form ref="formRef" :model="permissionForm" :rules="rules" label-width="130px">
        ...
        <el-form-item label="图标：" prop="icon">
          <el-input v-model="permissionForm.icon" placeholder="请选择图标" readonly @click="showIconDialog = true" />
    ...
    <el-dialog title="选择图标" v-model="showIconDialog" width="80%" class="icon-dialog">
      <el-input v-model="iconSearch" placeholder="搜索图标..." clearable prefix-icon="Search" class="icon-search-input" />
...
const loadPermissionTree = async () => {
  ...
  data.permissionTree = convertToTreeView(resp.data);
};
...
const savePermission = () => {
  formRef.value.validate(valid => {
    if (valid) {
      ...
      http.post("/api/permission/save", params).then(res => {
        if (res.code === 200) {
...
const remove = (node, row) => {
  ElMessageBox.confirm(`确认删除菜单【${row.permissionName}】？`, "提示", {
    ...
    http.post(`/api/permission/delete/${row.id}`).then(res => {
      if (res.code === 200) {
